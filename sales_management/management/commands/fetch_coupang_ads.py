...
class Command(BaseCommand):
    help = "쿠팡 광고페이지에서 날짜 범위(일별)로 엑셀 다운로드 후 DB 저장 (엑셀 삭제, Playwright 버전)"
    ...
    def handle(self, *args, **options):
        ...
        # (1) 광고 보고서 엑셀 다운로드 (Playwright 이용)
        self.login_and_download_ads(download_dir, start_dt, end_dt)
        # (2) 엑셀 → DB 저장 & 파일 삭제
        self.parse_and_store_ads_reports(download_dir)

        logger.info("[완료] fetch_coupang_ads 명령이 끝났습니다.")

    def login_and_download_ads(self, download_dir, start_d, end_d):
        ...
        with sync_playwright() as p:
            ...
            try:
                ...
                # (B) 광고 보고서 페이지 이동
                page.goto("https://advertising.coupang.com/marketing-reporting/billboard/reports/pa")
                page.wait_for_timeout(1000)

                # (B-1) 재인증 버튼 확인 (존재하면 클릭)
                try:
                    reauth_btn = page.wait_for_selector('//a[@href="/user/wing/authorization"]', timeout=3000)
                    if reauth_btn:
                        reauth_btn.click()
                ...
                # (C) '기간 설정' 라디오 클릭
                try:
                    period_label = page.wait_for_selector(
                        '//label[contains(@class,"ant-radio-wrapper") and .//span[text()="기간 설정"]]',
                        timeout=5000
                    )
                    page.evaluate("element => element.click()", period_label)
                    page.wait_for_timeout(1000)
                    classes = period_label.get_attribute("class")
                ...
                date_picker = page.wait_for_selector("div.ant-picker-input", timeout=10000)
                date_picker.click()
                page.wait_for_timeout(1000)
                ...
                try:
                    start_elem = page.wait_for_selector(start_xpath, timeout=5000)
                    start_elem.click()
                    page.wait_for_timeout(500)
                except Exception as e:
                ...
                try:
                    end_elem = page.wait_for_selector(end_xpath, timeout=5000)
                    end_elem.click()
                    page.wait_for_timeout(500)
                except Exception as e:
                ...
                try:
                    ok_btn = page.wait_for_selector("button.ant-picker-ok", timeout=5000)
                    ok_btn.click()
                    page.wait_for_timeout(1000)
                ...
                # (D) "일별" 라디오 선택
                try:
                    daily_label = page.wait_for_selector(
                        '//label[contains(@class,"ant-radio-wrapper") and .//span[text()="일별"]]',
                        timeout=5000
                    )
                    page.evaluate("element => element.click()", daily_label)
                    page.wait_for_timeout(1000)
                ...
                # (E) 캠페인 전체선택 -> 확인 버튼 클릭
                try:
                    camp_btn = page.wait_for_selector("button.campaign-picker-dropdown-btn", timeout=5000)
                    camp_btn.click()
                    page.wait_for_timeout(1000)
                    select_all_label = page.wait_for_selector(
                ...
                try:
                    radio_label = page.wait_for_selector(
                        '//label[contains(@class,"ant-radio-wrapper") and .//span[text()="캠페인 > 광고그룹 > 상품 > 키워드"]]',
                        timeout=5000
                    )
                    page.evaluate("element => element.click()", radio_label)
                    page.wait_for_timeout(1000)
                    label_class = radio_label.get_attribute("class")
                ...
                try:
                    ...
                    page.evaluate("element => element.click()", create_btn)
                ...
                try:
                    ...
                    row_elem = page.wait_for_selector(row_xpath, timeout=30000)
                    logger.info("row-index=0 행을 찾았습니다.")

                    try:
                        status_cell = row_elem.query_selector('.//div[@role="gridcell" and @col-id="status"]//div[contains(text(),"생성중")]')
                        if status_cell:
                            logger.info("row-index=0 행의 상태가 '생성중'임을 확인했습니다.")
                    except Exception as e:
                        logger.warning("row-index=0 행이 '생성중' 상태가 아닙니다. 혹시 이미 '생성 완료'인가요?")

                    # 대기: 상태 셀이 "생성 완료"로 바뀔 때까지 (최대 120초)
                    ...
                    page.evaluate("element => element.scrollIntoView(true)", row_elem)
                    page.wait_for_timeout(1000)
                    download_btn = row_elem.query_selector("xpath=.//button[contains(@class,'ant-btn-primary') and .//span[text()='다운로드']]")
                    if download_btn:
                        page.evaluate("element => element.scrollIntoView(true)", download_btn)
                        page.wait_for_timeout(1000)
