    ...
    async def login_and_download_coupang(self, download_dir, start_d, end_d):
        ...
        async with async_playwright() as p:
            browser = await p.chromium.launch(
                headless=True,  # 필요에 따라 headed 모드로 전환 (Xvfb 등 가상디스플레이 환경 필요)
            ...
            try:
                ...
                # 무조건 2차 인증 진행 (2FA 강제 실행)
                logger.info("무조건 2차 인증 실행: 2FA 진행을 강제합니다.")
                ...
                # 여기서 인증번호 추출 후 확인 버튼 클릭 등의 로직을 추가하면 됩니다.
